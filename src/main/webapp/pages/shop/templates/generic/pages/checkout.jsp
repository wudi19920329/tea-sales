...
<script type="text/html" id="subTotalsTemplate">
		{{#subTotals}}
			<tr id="cart-subtotal-{{code}}" class="cart-subtotal subt"> 									
				<td class="order-total-label">{{#discounted}}折扣&nbsp;-&nbsp;{{text}}{{/discounted}}{{^discounted}}{{title}}{{/discounted}}</td> 
...
function isFormValid() {
	...
	if(valid==false) {//disable submit button
		...
		$(formErrorMessageId).html('<!--<img src="<c:url value="/resources/img/icon_success.png"/>" width="40"/>&nbsp;--><strong>订单可以 提交</strong>');
		$(formErrorMessageId).show();
					...
					<div class="col-md-12">
						<div class="coupon-accordion">
							...
							<sec:authorize access="!hasRole('AUTH_CUSTOMER') and !fullyAuthenticated">
								<p class="muted common-row"><a href="<c:url value="/shop/customer/customLogon.html"/>">登录或注册网上购买来简化过程</a></p>
							</sec:authorize>					
						</div>
					</div>
				</div>
