...
import com.tea.dbc.DatabaseConnection;
...
public class GoodsHandle {
    ...
    public Goods findById(int id) throws Exception {
        ...
        if (rs.next()) {
            ...
            goods.setPrice(rs.getBigDecimal(6));
    ...
    public boolean doCreate(Goods good) throws Exception {
        ...
        pstmt.setBigDecimal(2, good.getPrice());
    ...
    public List<Goods> findAll(IntHolder num,int limitMin,int perPage) throws Exception {
        ...
        while (rs.next()) {
            ...
            good.setPrice(rs.getBigDecimal(7));
    ...
    public boolean doUpdate(Goods good) throws Exception {
        ...
        pstmt.setBigDecimal(6, good.getPrice());
    ...
    public List<Goods> findByCeta(int cetaId,IntHolder num,int limitMin,int perPage) throws Exception {
        List<Goods> all = new ArrayList<Goods>();
        String sql = "SELECT SQL_CALC_FOUND_ROWS id,num,content,type_id,image,producter_id,price,name,create_date,discount_price,discounted from goods where status=2 and type_id="+cetaId+" order by create_date desc limit ?,?";
        this.pstmt = this.conn.prepareStatement(sql);
        ...
        while (rs.next()){
            ...
            java.util.Date date=new  java.util.Date(timeStamp.getTime());
            good.setCreatDate(date);
            good.setDiscountPrice(rs.getBigDecimal(10));
            good.setDiscounted(rs.getBoolean(11));
            all.add(good);
        }
        rs = this.pstmt.executeQuery("SELECT FOUND_ROWS()");
        if(rs.next()){
