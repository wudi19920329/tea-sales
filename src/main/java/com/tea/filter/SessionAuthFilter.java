...
import com.tea.servlet.UserServlet;

@WebFilter(urlPatterns = { "/admin/product" }, filterName = "auth")
public class SessionAuthFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		if (request instanceof HttpServletRequest) {
			...
			User userSession = UserServlet.getOptionalLoginSession(req);
			if (userSession == null) {
				...
				response.getOutputStream()
						.write(MAPPER.writeValueAsString(new ExceptionInfo(String.valueOf(401), "未登陆"))
				...
				System.out.println(
						MessageFormat.format("Request allowed using LoginSession token={0}", userSession.getId()));
			}
		}
